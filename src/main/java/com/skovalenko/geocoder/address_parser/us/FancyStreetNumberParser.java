...
public class FancyStreetNumberParser {
	...
	// An alphanumeric House Number starting from char, like 'MI99','L3','L10201','N50W5200'
	private final static String STREET_NUMBER_ANUM = "^\\s*[a-zA-Z]+\\d+[a-zA-Z]?\\d*\\s+";
	// An alphanumeric House Number starting from number, like '1A1121','3554E','2C200'
	private final static String STREET_NUMBER_NUMA = "^\\s*\\d+[a-zA-Z]\\d*[a-zA-Z]?\\s+";
    private final static String FANCY_STREET_NUMBER_PATTERNS[] = { STREET_NUMBER_DBL, STREET_NUMBER_FRC, STREET_NUMBER_FRC2, STREET_NUMBER_LETR,
            STREET_NUMBER_BIS, STREET_NUMBER_ROMA, STREET_NUMBER_ANUM, STREET_NUMBER_NUMA };

    public static String parse(String source) {
        String result = handleNumberSign(source);
        for (int i = 0; i < FANCY_STREET_NUMBER_PATTERNS.length; i++) {
            result = handleFancyStreetNumber(result, FANCY_STREET_NUMBER_PATTERNS[i]);
        }
        return result;
    }
	 ...
	 * 10-b, 14bis, 435-II, 35-3, 156/5, D1, 315A, 350-1, N50W5200, 2C200, 315 1/2.
	 * @param addressLine address line to process (correct building number leaving just first numbers)
    ...
    private static String getFirstTokenAfterMatch(String text, String pattern) {
        Matcher matcher = Pattern.compile(pattern).matcher(text);
        if (matcher.find()) return new StringTokenizer(text.substring(matcher.end()).trim()).nextToken();
        return text;
    }
}
