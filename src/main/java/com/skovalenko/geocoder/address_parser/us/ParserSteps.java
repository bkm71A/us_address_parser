...
public class ParserSteps {
    public static AddressToken[] findNumericWords(AddressToken[] addressTokens) {
        ...
        for (int x = 0; x < Math.min(1, addressTokens.length); x++) {
            if (UsAddressParserDataStrings.NUMERIC_WORDS.contains(addressTokens[x].getToken())) {
    ...
    public static AddressToken[] findDirection(AddressToken[] addressTokens) {
        for (int x = 0; x < addressTokens.length; x++) {
            if (UsAddressParserDataStrings.DIRECTION.containsKey(addressTokens[x].getToken())) {
                // quickfix handle '4638 North','43 West' addresses
                if (x == addressTokens.length - 1 && addressTokens.length < 4
                        && UsAddressParserDataStrings.STREET_NAMES_EXCL.contains(addressTokens[x].getToken())) {
                } else {
    ...
    public static AddressToken[] findSubUnitType(AddressToken[] addressTokens) {
        for (int x = 0; x < addressTokens.length; x++) {
            if (UsAddressParserDataStrings.SU_TYPES.containsKey(addressTokens[x].getToken())) {
    ...
    public static AddressToken[] findStreetType(AddressToken[] addressTokens) {
        for (int x = addressTokens.length - 1; x > 1; x--) {
            if (UsAddressParserDataStrings.STREET_TYPE.containsKey(addressTokens[x].getToken())) {
        ...
        if (UsAddressParserDataStrings.ROUTE_ALIASES.contains(addressTokens[addressTokens.length - 1].getToken()) && addressTokens.length > 2) {
            addressTokens[addressTokens.length - 1].setHint(HintTypes.HINT_STREET_TYPE);
    ...
    public static AddressToken[] categorizeNumbers(AddressToken[] addressTokens) {
        for (int x = 0; x < addressTokens.length; x++) {
            if (addressTokens[x].isPartialNumeric()) {
                if (x > 0) {
                    if ((addressTokens[x - 1].getHint() == HintTypes.HINT_CITY) | (addressTokens[x - 1].getHint() == HintTypes.HINT_STATE)) {
                        ...
                        if ((addressTokens[x].getHint() == HintTypes.HINT_SU_TYPE)) {
                            addressTokens[x + 1].setHint(HintTypes.HINT_SU_NUMBER);
                        } else {
    ...
    public static AddressToken[] groupNoHits(AddressToken[] addressTokens) {
        int lastStreetName = ParserUtilities.findLastHintPosition(addressTokens, HintTypes.HINT_STREET_NAME);
        Vector<AddressToken> at = vectroFromArray(addressTokens);
        for (int x = 0; x < at.size(); x++) {
            if (at.get(x).getHint() == HintTypes.HINT_NONE) {
                ...
                for (int y = x + 1; y < at.size(); y++) {
                    ...
                    } else if ((at.get(y).getHint() == HintTypes.HINT_STREET_DIRECTION) && y < lastStreetName) {
    ...
    public static AddressToken[] categorizeNoHits(AddressToken[] addressTokens) {
        int streetTypePos = ParserUtilities.findFirstHintPosition(addressTokens, HintTypes.HINT_STREET_TYPE);
        if (streetTypePos == -1) {
    ...
    public static AddressToken[] fixBadMatches(AddressToken[] addressTokens) {
        if (ParserUtilities.getValueFromTokens(addressTokens, HintTypes.HINT_STREET_NAME) == "") {
            for (int x = 0; x < addressTokens.length; x++) {
                if (addressTokens[x].getHint() == HintTypes.HINT_STREET_TYPE) {
                    addressTokens[x].setHint(HintTypes.HINT_NONE);
                }
    ...
    private static Vector<AddressToken> vectroFromArray(AddressToken addressToken[]) {
            ...
            for (int x = 0; x < addressTokens.length; x++) {
                if (addressTokens[x].getHint() == HintTypes.HINT_STREET_NAME) {
                    if (first) {
                        ...
                        newTokens[newTokensIdx].setHint(HintTypes.HINT_STREET_NAME);
                    } else {
